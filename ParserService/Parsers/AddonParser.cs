...
public class AddonParser : IParser<DataAddon>
{
        ...
        try
        {
            ...
            for (int attempt = 0; attempt < retryCount; attempt++)
            {
                try
                {
                    ...
                    var responseTr = httpClientTr.PostAsync(urlTr, content).Result;
    ...
    public Task<DataAddon> ParseAsyncJson(
        string url,
        string conceptId,
    ...
    public async Task<DataAddon> ParseProductAsyncJson(
        string url,
        string urlTr,
        string productId,
        string productIdTr,
    ...
    )
    {
        try
        {
            ...
            var requestBodyTr = new
            {
                operationName = "productRetrieveForCtasWithPrice",
                variables = new { productId = productIdTr },
                extensions = new
                {
                    persistedQuery = new
                    {
            ...
            var jsonBody = JsonSerializer.Serialize(requestBody);
            var jsonBodyTr = JsonSerializer.Serialize(requestBodyTr);

            var content = new StringContent(jsonBody, Encoding.UTF8, "application/json");
            var contentTr = new StringContent(jsonBodyTr, Encoding.UTF8, "application/json");
            int retryCount = 3; // Количество повторных попыток
            int delayMilliseconds = 500; // Задержка между попытками
            ...
            for (int attempt = 0; attempt < retryCount; attempt++)
            {
                try
                {
                    ...
                    var responseTr = httpClientTr.PostAsync(urlTr, contentTr).Result;
                    responseTr.EnsureSuccessStatusCode();
                    var jsonTr = await responseTr.Content.ReadAsStringAsync();
                    // Читаем JSON-ответ
                    var resultTr = JsonSerializer.Deserialize<RootobjectTrAddon>(
